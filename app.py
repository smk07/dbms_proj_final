...
def lictest():
    ...
    conn=connect_db()
    cur=conn.cursor()
    sql=f'''select e_id,city from rto_emp order by city'''
    cur.execute(sql)
    curr=cur.fetchall()
    cities=[]
    for i in curr:
    ...
    if form.validate_on_submit():
        ...
        if llr_id not in llr:
            ...
            form.llr_id.data=''
            return render_template('ltest.html',form=form,cities=cities)

        sql=f'''select l_id from lic_reg where l_id='{llr_id}' '''
        cur.execute(sql)
        curr=cur.fetchall()
        if curr:
            flash("You have already passed the test and your license has been issued!")
            return render_template('ltest.html',form=form,cities=cities)
